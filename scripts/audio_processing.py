...
def load_audio(file_path):
    """
    Audio loading function that tries multiple methods
...
def get_audio_files_paths():
    ...
    e.g., Lorita_approve.m4a, Wagner_confirm.mp3
    """
    audio_data = {}
...
def main():
    ...
    audio_data = get_audio_files_paths()
    ...
    for member, audio_files in audio_data.items():
        ...
        for phrase in PHRASES:
            if phrase not in audio_files:
                print(f" Warning: No '{phrase}' file found for {member}")
            ...
            if phrase_features:
                ...
                print(f"    X No features extracted for {member}-{phrase}")
                processing_stats['files_failed'] += 1
